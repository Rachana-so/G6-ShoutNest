    ...
    if request.method == 'GET':
        if UserId != 0:
            ...
            for i in friends_set:
                friends=User.objects.filter(userId=i)
...
@csrf_exempt
def pendingRequestsApi(request,UserId=0):
    if request.method == 'GET':
        if UserId != 0:
            friends=Friends.objects.filter(userId=UserId) | Friends.objects.filter(friendId=UserId)
            friends_serializers = FriendsSerializer(friends,many=True)
...
@csrf_exempt
def getSuggestions(request, UserId=0):
    if request.method == 'GET':
        if UserId != 0:
            friends=Friends.objects.filter(userId=UserId) | Friends.objects.filter(friendId=UserId)
            friends_serializers = FriendsSerializer(friends,many=True)
            friend_list1=friends_serializers.data
            print('friendlist',friend_list1)
            friends_set1=set()
            for i in friend_list1:
                if i['status']==3:
                    friends_set1.add(i['friendId'])
            for i in friend_list1:
                if i['status']==3:
                    friends_set1.add(i['userId'])
            print('friendset',friends_set1)
            ...
            print('all users',userlist2)

            """ user1_serializer = UserSerializer(userlist2,many=True)
            userlist3=user1_serializer.data
            print("userlist3 serialised ",userlist3) """
           
           

            """ user_suggested=User.objects.all()
            user1_serializer = UserSerializer(user_suggested,many=True)
            userlist2.append(user1_serializer.data)
            """
            for i in userlist2:
                userlist_set1.add(i)
            print("set",userlist_set1)
           
            for i in friends_set1:
                print(i)
                pk=User.objects.get(userId=i)
                userlist_set1.remove(pk)

                
                
                print("removed",userlist_set1)
                """ userlist_set1.remove(i['userId'])
                    userlist2.filter(userId=i).delete()
                    userlist2.exclude(userId=i) 
                """
             
                
            user_serializer = UserSerializer(userlist_set1,many=True)
            suggestionlist.append(user_serializer.data)
            print('suggestion ',suggestionlist)
            


        return JsonResponse(suggestionlist,safe=False)
    return JsonResponse(" not found..",safe=False)
